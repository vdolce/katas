...
##the given numbers finds one that is different in evenness, and
...
def iq_test(numbers):
    # boolean array
    res = list(map(lambda x: int(x)%2==0, numbers.split()))
 
    if(res.count(True) == 1):
        return res.index(True) + 1
    else:
        return res.index(False) + 1

print(iq_test("2 4 7 8 10"))
#print(iq_test("1 2 5 1"))
